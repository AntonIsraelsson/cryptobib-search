...
function flattenVenue(e: { parent?: unknown }): string {
...
async function main() {
	...
	try {
		yamlContent = await readFile(yamlFile, 'utf8');
	} catch {
		console.log('cryptobib.yaml not found, using test data...');
		yamlFile = 'data/test.yaml';
		yamlContent = await readFile(yamlFile, 'utf8');
	}
	const y = yaml.load(yamlContent) as Record<string, HayEntry> | null;

	if (!y || typeof y !== 'object') {
		throw new Error(`Failed to parse YAML from ${yamlFile}`);
	}
	...
	for (const [key, e] of Object.entries(y)) {
		...
		const original_hayagriva = yaml.dump(
			{ [key]: e },
			{
				indent: 2,
				lineWidth: -1,
	...
	function finalizeTerms(terms: string[], maps: Map<number, PostRec>[]) {
		...
		for (let oldId = 0; oldId < idx.length; oldId++) {
			...
			maps.forEach((m, k) => {
				...
				if (rec) (newMaps[k] as Map<number, PostRec>).set(newId, rec);
			});
		}
	...
	for (const d of entries) {
		...
		const obj = {
			...
			key: d.key,
			title: d.title,
			authors_str: d.authors_str,
			venue: d.venue,
			year: d.year,
			page_range: d.page_range,
			doi: d.doi,
			url: d.url,
			original_hayagriva: d.original_hayagriva
		};
	...
	async function writeToSearchDirs(filename: string, data: Uint8Array | string) {
	...
	const ptrsBuf = new Uint8Array(
		corePosts.ptrs.titlePtrStart.byteLength +
			corePosts.ptrs.titlePtrLen.byteLength +
			corePosts.ptrs.authorsPtrStart.byteLength +
			corePosts.ptrs.authorsPtrLen.byteLength +
			corePosts.ptrs.keyPtrStart.byteLength +
			corePosts.ptrs.keyPtrLen.byteLength
	);
	let po = 0;
	function append(arr: Uint32Array) {
		ptrsBuf.set(new Uint8Array(arr.buffer), po);
		po += arr.byteLength;
	}
	append(corePosts.ptrs.titlePtrStart);
	append(corePosts.ptrs.titlePtrLen);
	append(corePosts.ptrs.authorsPtrStart);
	append(corePosts.ptrs.authorsPtrLen);
	append(corePosts.ptrs.keyPtrStart);
	append(corePosts.ptrs.keyPtrLen);
	...
	const extPtrsBuf = new Uint8Array(
		extPosts.ptrs.venuePtrStart.byteLength +
			extPosts.ptrs.venuePtrLen.byteLength +
			extPosts.ptrs.yearPtrStart.byteLength +
			extPosts.ptrs.yearPtrLen.byteLength +
			extPosts.ptrs.doiPtrStart.byteLength +
			extPosts.ptrs.doiPtrLen.byteLength
	);
	po = 0;
	append(extPosts.ptrs.venuePtrStart);
	append(extPosts.ptrs.venuePtrLen);
	append(extPosts.ptrs.yearPtrStart);
	append(extPosts.ptrs.yearPtrLen);
	append(extPosts.ptrs.doiPtrStart);
	append(extPosts.ptrs.doiPtrLen);
