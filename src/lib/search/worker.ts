...
function readPostings(
	...
	while (p < end) {
		let v;
		[v, p] = uvarint(postings, p);
		const docId = lastDoc + v;
		lastDoc = docId;
		if (withPositions) {
			let nPos;
			[nPos, p] = uvarint(postings, p);
			let lastPos = 0;
			const posArr: number[] = new Array(nPos);
			for (let k = 0; k < nPos; k++) {
				let d;
				[d, p] = uvarint(postings, p);
				lastPos += d;
				posArr[k] = lastPos;
			}
			docs.push(docId);
...
async function loadCore() {
	...
	let o = 0;
	const numTerms = dictView.getUint32(o, true);
	o += 4;
	const termBytesLen = dictView.getUint32(o, true);
	o += 4;
	const termOffsets = new Uint32Array(dictBuf as ArrayBuffer, o, numTerms + 1);
	o += 4 * (numTerms + 1);
	...
	function readU32Array(n: number): Uint32Array {
		const arr = new Uint32Array(ptrsBuf as ArrayBuffer, p, n);
		p += 4 * n;
		return arr;
	}
...
async function handleInit(msg: InitMsg) {
	...
	await loadCore();
	state.inited = true;
	(self as unknown as Worker).postMessage({ type: 'init:ok' });
}
...
async function handleSearch(q: string, opts: SearchOpts = {}) {
	...
	if (tokens.length === 0 && phrases.length === 0) {
		(self as unknown as Worker).postMessage({ type: 'search:ok', results: [] });
		return;
	}
	...
	function resolveTermRanges(token: string, tier: 'core' | 'ext', prefix: boolean): number[] {
		...
		const exactMatch =
			exactId < termOffsets.length - 1 && termAt(termBlob, termOffsets, exactId) === token;
	...
	function postingsForCoreTerm(
		termId: number
	): { docs: number[]; src: 'title' | 'authors' | 'key'; positions?: number[][] }[] {
	...
	function mergeSortedUnique(arrs: number[][]): number[] {
		...
		while (true) {
			...
			for (let j = 0; j < iters.length; j++) {
				const it = iters[j];
				if (it.i < it.a.length && it.a[it.i] < min) {
					min = it.a[it.i];
					minJ = j;
				}
			}
	...
	for (let t = 0; t < tokens.length; t++) {
		...
		for (const id of coreIds) {
			for (const r of postingsForCoreTerm(id)) {
				docLists.push(r.docs);
				srcs.add(r.src);
			}
		}
		for (const id of extIds) {
			for (const r of postingsForExtTerm(id)) {
				docLists.push(r.docs);
				srcs.add(r.src);
			}
		}
	...
	(self as unknown as Worker).postMessage({ type: 'search:ok', results: out, tookMs: t1 - t0 });
}
...
self.onmessage = async (ev: MessageEvent<AnyMsg>) => {
	...
	try {
		if (msg.type === 'init') await handleInit(msg);
			...
			const id =
				typeof msg.idOrKey === 'string' ? (state.keyToId.get(msg.idOrKey) ?? -1) : msg.idOrKey;
			const e = getEntryById(id);
			(self as unknown as Worker).postMessage({ type: 'get:ok', entry: e });
		}
	} catch (error: unknown) {
